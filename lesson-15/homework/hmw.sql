...
Select emp.id, name, department_name from employees as emp
...
Select cus.customer_id, name from Customers as cus
...
Select category_id, max(id) ID, max(product_name) PRNAme, max(price) MAXPRice from products 
...
(3, 102, 90),
(1, 102, 80);
select course_id, max(student_id), max(grade) from grades
group by course_id
