...
Select CONCAT_WS('', EMPLOYEE_ID, '-', FIRST_NAME, ' ', LAST_NAME) as FullName from Employees
...
Select Substring (Stuff(PHONE_NUMBER, 5,3, '999'), 5, 3) as Phn from Employees
...
Select FIRST_NAME as Name, len(FIRST_NAME) Lenght from Employees
where FIRST_NAME like 'A%' or FIRST_NAME like  'J%' or FIRST_NAME like 'M%'
Order by FIRST_NAME

----Write an SQL query to find the total salary for each manager ID.(Employees table)
Select MANAGER_ID, sum(SALARY) TotSal from Employees
Group by MANAGER_ID
...
Select Year1, GREATEST(MAx1, Max2, Max3) MaxNumber from TestMax
...
Select id, coalesce(ssn, passportid, itin) as Non_null from Person

----Medium Tasks
----Split column FullName into 3 part ( Firstname, Middlename, and Lastname).(Students Table)

Select StudentID, Parsename(replace (FullName, ' ', '.'), 3) Firstname, 
		  Parsename(replace (FullName, ' ', '.'), 2) Middlename,
		  Parsename(replace (FullName, ' ', '.'), 1) Lastname
		from Students
...
Select Ord.CustomerID,  Ord.OrderID, Ord.DeliveryState, Ord.Amount from Orders Ord
join Orders as Ord2
on Ord.CustomerID=Ord2.CustomerID
where Ord.DeliveryState<>Ord2.DeliveryState and Ord.DeliveryState= 'TX'

----Write an SQL statement that can group concatenate the following values.(DMLTable)

Select STRING_AGG(String, ' ') as Formula from DMLTable

----Find all employees whose names (concatenated first and last) contain the letter "a" at least 3 times.

Select Concat_WS (' ', FIRST_NAME, LAST_NAME),  PATINDEX ('%a%a%a%', Concat_WS (' ', FIRST_NAME, LAST_NAME) ) As FullName from Employees
where PATINDEX ('%a%a%a%', Concat_WS (' ', FIRST_NAME, LAST_NAME) ) > 0
...
Select DEPARTMENT_ID, count(EMPLOYEE_ID) Employees, (count(GETDATE() - year(HIRE_DATE))) / count(EMPLOYEE_ID) *100 as HirePercentage from Employees
...
Select JobDescription , Max(MissionCount) MAxMis, Min(MissionCount) MinMis from Personal
Group by JobDescription

----Difficult Tasks
----Write an SQL query that separates the uppercase letters, lowercase letters, numbers, and 
...
SELECT
  (SELECT STRING_AGG(ch, '') FROM Chars WHERE ch LIKE '[A-Z]') AS UppercaseLetters,
  (SELECT STRING_AGG(ch, '') FROM Chars WHERE ch LIKE '[a-z]') AS LowercaseLetters,
  (SELECT STRING_AGG(ch, '') FROM Chars WHERE ch LIKE '[0-9]') AS Numbers,
  (SELECT STRING_AGG(ch, '') FROM Chars WHERE ch LIKE '[^a-zA-Z0-9]') AS Others;
...
Select Substring(Equation, 1, 1), 
		Substring(Equation, 2, 1), 
		Substring(Equation, 3, 1)
		from Equations
