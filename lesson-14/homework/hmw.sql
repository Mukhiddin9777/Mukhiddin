...
Select ID, Substring (Name, 1, (PATINDEX('%,%', Name)-1)) as Name, 
		Substring (Name, (PATINDEX('%,%', Name)+1), len(name)) as Surname from TestMultipleColumns
...
Select REPLACE (cast('1234ABC123456XYZ1234567890ADS' as varchar), cast('0123456789' as varchar), '[X]')

DECLARE @str VARCHAR(MAX) = '1234ABC123456XYZ1234567890ADS';

SELECT 
    @str AS OriginalString,
    TRANSLATE(@str, '0123456789', 'XXXXXXXXXX') AS ReplacedString;
...
Select *, len(texts) - len (REPLACE(texts, ' ', '')) Space from CountSpaces
...
Select Employee_ID, First_Name, Last_Name, Year(GETDATE())-Year(HIRE_DATE) As Hiredate, Year(Hire_Date) from Employees
...
Select  Substring('rtcfvty34redt', 1, (PATINDEX ('%[0-9]%', 'rtcfvty34redt'))-1), 
		Substring('rtcfvty34redt', (PATINDEX ('%[0-9]%', 'rtcfvty34redt')), LEN('rtcfvty34redt'))
...
Select PATINDEX ('%@%','sdgfhsdgfhs@121313131')-1,   

DECLARE @string VARCHAR(MAX) = 'sdgfhsdgfhs@121313131';


WITH CTE AS (
    SELECT 1 AS n, SUBSTRING(@string, 1, 1) AS Char
    UNION ALL
    SELECT n + 1, SUBSTRING(@string, n + 1, 1)
    FROM CTE
...
Select p1.id, case when p1.code = 0 THEN p2.code 
		  ELSE p1.code 
		   END AS code
		   from p1
...
Select *, case when Year(HIRE_DATE)<1 then 'New Hire'
			when Year(HIRE_DATE) between 1 and 5 then 'Junior'
			when Year(HIRE_DATE) between 6 and 10 then 'Mid-Level'
			when Year(HIRE_DATE) between 11 and 20 then 'Senior'
			else 'Veteran'
			end as Employer_Stage
			from Employees
...
DECLARE @string VARCHAR(MAX) = 'ab,cd,ef';
...
WITH Split AS (
    ...
    SELECT 
        CASE 
            WHEN LEN(value) >= 2 THEN RIGHT(LEFT(value, 2), 1) + LEFT(value, 1) + SUBSTRING(value, 3, LEN(value))
            ELSE value
        END AS swapped_value
    FROM Split
)
SELECT STRING_AGG(swapped_value, ',') AS result
FROM Swapped;
...
FROM (
    SELECT *,
           ROW_NUMBER() OVER (PARTITION BY PlayerID ORDER BY EventDate ASC) AS rn
    FROM Activity
) AS ranked
WHERE rn = 1;
...
SELECT 
    ...
    s.DayName,
    s.Amount,
    ROUND(CAST(s.Amount AS FLOAT) / t.TotalAmount * 100, 2) AS Percentage
FROM SalesWithWeek s
JOIN TotalPerWeekArea t
    ON s.Area = t.Area AND s.WeekNum = t.WeekNum
ORDER BY Area, WeekNum, DayName;
