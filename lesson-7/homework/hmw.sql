...
Select count(distinct Category) as CountCategory from Products
...
Select ProductID,sum (SaleAmount) SumSale, avg (SaleAmount) Avgsale
from Sales
Group by ProductID

----Write a query that uses COUNT(columnname) to count the number of employees from the Department HR.
Select DepartmentName, COUNT(EmployeeId) CountEmployee
from Employees
Group by DepartmentName
...
Select DepartmentName, max(Salary) MaxSalary, min(Salary) MinSalary
from Employees
Group by DepartmentName
...
Select DepartmentName, avg(Salary) AvgSalary
from Employees
Group by DepartmentName
...
Select DepartmentName, avg(Salary) AvhSalary, count(*) Employees
from Employees
Group by DepartmentName
...
Select Year(SaleDate) Saleyear, sum (SaleAmount) SumSale
from Sales
Group by SaleDate

----Write a query that uses COUNT to show the number of customers who placed at least 3 orders.
Select CustomerID, COUNT (*) As CountCustomers 
from Orders
Group by CustomerID
having Count (*) >=3
...
Select DepartmentName,  Sum(Salary) CountEmployee from Employees
Group by DepartmentName
...
Select ProductID, Avg(SaleAmount) Avgsale
from Sales
Group by ProductID
...
Select CustomerID, Sum(SaleAmount) SumSale
from Sales
Group by CustomerID
...
Select DepartmentName, Sum(Salary) SumSalary, Avg(Salary) AvgSalary 
from Employees
Group by DepartmentName
...
Select CustomerID, max (TotalAmount) As MAxAmount, min (TotalAmount) As MinAmount
from Orders
Group by CustomerID
...
Select Month(OrderDate) Monthdate,  Sum(TotalAmount) SumAmount, count(distinct ProductID)
from Orders
...
Select Year(OrderDate) Yeardate,  MAx(quantity) MAxquant, Min(quantity) Minquant
from Orders
