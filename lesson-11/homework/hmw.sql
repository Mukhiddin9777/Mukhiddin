...
Select OrderID, CONCAT_WS(' ', FirstName, LastName) as CustomerName, OrderDate from Orders as Ord
...
Select Emp.Name as EmployeeName, DepartmentName from Employees as Emp
...
Select Emp.Name as TopEmployeeName, DepartmentName, Salary as MaxSalary from Employees as Emp
join Departments as Dep
on Emp.DepartmentID=Dep.DepartmentID
...
Select CONCAT_WS(' ', FirstName, LastName), OrderID, OrderDate from Customers as Cus
join Orders as ord
...
Select CONCAT_WS(' ', FirstName, LastName) as CustomerName, count(Quantity) as Totalorders from Orders as Ord
...
Select ProductName, SupplierName from Products as Pr
...
Select CONCAT_WS(' ', FirstName, LastName) as CustomerName, Max(Ord.OrderDate) as MostRecentOrderDate, max(Ord.OrderID) from Customers as Cus
...
Select CONCAT_WS(' ', FirstName, LastName) as CustomerName, max(OrderID) OrderID, sum(Totalamount) Ordertotal from Orders as Ord
...
Select ProductName, SaleDate, SaleAmount from Products as Pr
join Sales as Sal
...
Select ProductName, sum(SaleAmount) TotalSalesAmount from Products as Pr
...
Select Emp.Name as EmployeeName, DepartmentName, Salary from Employees as Emp
join Departments as Dep
on Emp.DepartmentID=Dep.DepartmentID
...
Select ProductName, SaleDate, StockQuantity from Products as Pr
left join Sales as Sal
on Pr.ProductID=Sal.ProductID
...
Select Emp.Name as EmployeeName, DepartmentName, HireDate
...
Select CONCAT_WS(' ', FirstName, LastName) as CustomerName, OrderID, [Address], OrderDate from Customers as Cus
...
Select CONCAT_WS(' ', FirstName, LastName) as CustomerName, City, OrderID, TotalAmount as Amount from Customers as Cus
join Orders as Ord
on Cus.CustomerID=Ord.CustomerID
...
Select Emp.Name as EmployeeName, DepartmentName
 from Employees as Emp
join Departments as Dep
on Emp.DepartmentID=Dep.DepartmentID
where DepartmentName in ('Human Resources', 'Finance') or (LEN(Name) - LEN(REPLACE(LOWER(Name), 'a', '')) +
...
Select ProductName, SaleAmount as QuantitySold, Price
 from Products as Pr
join Sales as Sal
on Pr.ProductID=Sal.ProductID
Where SaleAmount >  100 and Price>500
...
Select Emp.Name as EmployeeName, DepartmentName, Salary
 from Employees as Emp
join Departments as Dep
on Emp.DepartmentID=Dep.DepartmentID
