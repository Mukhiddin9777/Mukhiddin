...
Select category, Count (*) as CountCategory  from Products
Group By category
...
Select *, iif (StockQuantity<30,'Low Stock', 'Sufficient') As Assessment
from Products

----Using Customers table, find the total number of customers in each country.
Select Country, Count (*) As NumberCustomers From Customers 
Group by Country

----Using Orders table, find the minimum and maximum quantity ordered.
Select Min(Quantity) MinQuant, Max(Quantity) MinQuant From Orders 
...
Select Year(Orderdate) Years, avg(quantity) AVGQUANT from Orders
...
Select ProductName, case when Price < 100 then 'Low'
				when Price between 100 and 500 then 'high'
...
Select ProductID, sum (Saleamount) SumSale
From Sales
Group by ProductID
...
Select Top 5 ProductID, Sum(Quantity) SumQuant
From Orders
Group by ProductID
