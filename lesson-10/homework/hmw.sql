...
Select Name as EmployeeName, Salary, DepartmentName from Employees as Emp
...
Select FirstName, LastName, OrderDate Name from Customers as Cus
join Orders as Ord
...
Select Name as EmployeeName, DepartmentName from Employees as Emp
...
Select SupplierName, ProductName from Products as Pr
...
Select Ord.OrderID, OrderDate, PaymentDate, Amount from Orders as Ord
...
Select Name as StudentName, CourseName from Students as St
join Enrollments as En
on St.StudentID=En.StudentID
join Courses as Co
on Co.CourseID=En.CourseID
...
Select FirstName, LastName, Quantity Name from Customers as Cus
join Orders as Ord
on Cus.CustomerID=Ord.CustomerID
where Quantity>3
...
Select Name as DepartmentName, count(EmployeeID) as Employeecount from Employees as Emp
join Departments as Dep
on Emp.DepartmentID=Dep.DepartmentID
group by Name
...
Select Pr.ProductID, ProductName from Products as Pr
left join Sales as Sal
on Pr.ProductID=Sal.ProductID
...
Select FirstName, LastName, count(OrderID) as TotalOrders from Customers as Cus
join Orders as Ord
on Cus.CustomerID=Ord.CustomerID
...
Select Emp.Name as Employee1, Man.Name as Employee2, Emp.ManagerID from Employees as Emp
join Employees as Man
on Emp.ManagerID=Man.ManagerID
where Emp.EmployeeID > Man.EmployeeID
...
Select OrderID, OrderDate, FirstName, LastName from Orders as Ord
...
Select Pay.OrderID, OrderDate, PaymentDate, Amount from Orders as Ord
...
Select Pro.ProductID, ProductName from Products as Pro
...
Select Name as EmployeeName, cast(Salary as float) as Salary from Employees 
...
Select Ord.OrderID, OrderDate from Orders as Ord
left join Payments as Pay
on Ord.OrderID=Pay.OrderID
where Year(OrderDate)<2020 and PaymentMethod is null

----Using the Products and Categories tables, write a query to return products that do not have a matching category.
----🔁 Expected Output: ProductID, ProductName?????????


Select Pro.ProductID, Pro.ProductName from Products as Pro
left join Categories as Cat
on Pro.Category=Cat.CategoryID
...
where Emp1.Salary>60000 and Emp2.Salary>60000  and Emp1.EmployeeID>Emp2.EmployeeID
...
Select SaleID, ProductName, SaleAmount from Products as Pr
join Sales as Sal
on Pr.ProductID=Sal.ProductID
...
Select St.StudentID, Name as StudentName from Students as St
join Enrollments as En
on St.StudentID=En.StudentID
join Courses as Co
on Co.CourseID=En.CourseID
...
Select Ord.OrderID, OrderDate, PaymentID from Orders as Ord
left join Payments as Pay
on Ord.OrderID=Pay.OrderID
...
Select ProductID, ProductName, CategoryName from Products as Pr
join Categories as Cat
on Pr.Category=Cat.CategoryID
